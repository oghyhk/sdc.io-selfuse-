...
const AI_TYPE_CONFIG = {
    searcher: {
        ...
        safeZoneWeight: 10,
        combatZoneWeight: 24,
        highValueZoneWeight: 86,
        highValueFightBias: 92,
        highValueCrateBias: 260,
    },
    runner: {
        crateSeekChance: 0.72,
        richCrateBias: 1.12,
        ...
        highValueFightBias: -45,
        highValueCrateBias: 0,
        safeZoneCrateBias: 220,
        safeZoneCrateSeekChance: 0.92,
    },
    fighter: {
        crateSeekChance: 0.54,
        richCrateBias: 1.06,
        ...
        safeZoneWeight: 8,
        combatZoneWeight: 38,
        highValueZoneWeight: 74,
        highValueFightBias: 128,
        highValueCrateBias: 170,
    },
};
...
function buildBotLoadout(difficulty = 'advanced', aiProfile = createAIOperatorProfile(difficulty)) {
    ...
    // Boss: always red ammo + red consumable, 300 each
...
export class AIPlayer extends Player {
    ...
    _getZoneAtPosition(context, x, y) {
        const col = Math.floor(x / TILE_SIZE);
        const row = Math.floor(y / TILE_SIZE);
        return context.mapData?.zones?.[row]?.[col] ?? null;
    }

    _hasNearbyThreats(context, radius = null) {
        const threatRadius = radius ?? (this.aiType === 'runner' ? 220 : 190);
        return [context.player, ...(context.aiPlayers || []).filter((bot) => bot.id !== this.id)]
            .filter((entity) => entity?.alive)
            .filter((entity) => !this.isFriendlyWith(entity))
            .some((entity) => dist(this.x, this.y, entity.x, entity.y) < threatRadius);
    }
    ...
    _pickTarget(context) {
        ...
        for (let si = 0; si < 3; si++) {
            ...
            const candidateZone = this._getZoneAtPosition(context, candidate.x, candidate.y);
            if (candidateZone === ZONE.HIGH_VALUE) {
                score += this.aiTypeConfig.highValueFightBias || 0;
            }
    ...
    _pickRoamDestination(context) {
        ...
        const safeZoneCrates = richCrates.filter((crate) => this._getZoneAtPosition(context, crate.x, crate.y) === ZONE.SAFE);
        const isRunnerIdle = this.aiType === 'runner' && !this._hasNearbyThreats(context, 250);
        const runnerSafeSeekChance = this.aiTypeConfig.safeZoneCrateSeekChance || this.aiTypeConfig.crateSeekChance;
        const candidateCrates = isRunnerIdle && safeZoneCrates.length ? safeZoneCrates : richCrates;
        const seekChance = isRunnerIdle && safeZoneCrates.length ? runnerSafeSeekChance : this.aiTypeConfig.crateSeekChance;
        if (candidateCrates.length && Math.random() < seekChance) {
            const crate = candidateCrates.reduce((best, current) => {
                if (!best) return current;
                const scoreCrate = (crate) => {
                    const zone = this._getZoneAtPosition(context, crate.x, crate.y);
                    let score = crate.items.reduce((sum, item) => sum + (item.sellValue || 0), 0) - dist(this.x, this.y, crate.x, crate.y) * (2 - this.aiTypeConfig.richCrateBias);
                    if (zone === ZONE.HIGH_VALUE) score += this.aiTypeConfig.highValueCrateBias || 0;
                    if (zone === ZONE.SAFE) score += this.aiTypeConfig.safeZoneCrateBias || 0;
                    return score;
                };
                const bestScore = scoreCrate(best);
                const currentScore = scoreCrate(current);
    ...
    _shouldUseConsumable(context) {
        ...
        return !this._hasNearbyThreats(context);
    }
