    ...
    drawCrates(crates, nearbyCrateId, openCrateId) {
        ...
        for (const crate of crates) {
            ...
            if (crate.tier === 'safe' && this.safeImage.complete && this.safeImage.naturalWidth > 0) {
                const imgScale = 1.6;
                // Preserve the source image aspect ratio so the safe doesn't look squashed.
                const aspect = this.safeImage.naturalWidth / this.safeImage.naturalHeight;
                const imgH = CRATE_HEIGHT * imgScale * 1.4;
