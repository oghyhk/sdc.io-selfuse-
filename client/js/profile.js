...
// ITEM_DEFS is populated exclusively by loadRuntimeDevConfig() from dev-config.json.
// This ensures dev-config.json is the SOLE source of truth for all item definitions.
// If loadRuntimeDevConfig() fails, ITEM_DEFS remains empty — the game will have no
// purchasable items, which is correct behavior for a server-authoritative game.
...
/**
 * Compute the documented loss multiplier using the victim's own ELO bracket.
 * killerElo is ignored and kept only to preserve existing call sites.
 */
...
export function computeEloChange(difficulty = 'advanced', extracted = false, eloKillBonus = 0, deathPenaltyScale = 1.0, playerElo = 1000) {
    ...
    const lossMult = Number.isFinite(Number(deathPenaltyScale))
        ? Number(deathPenaltyScale)
        : getLossMultiplier(playerElo);
    if (extracted) {
    ...
    return Math.round(-netLoss * lossMult);
}
...
export class ProfileStore {
    ...
    async _runServerProfileAction(action, payload = {}) {
        ...
        const runRequest = () => apiFetch('/profile-action', {
            method: 'POST',
            body: JSON.stringify({
        ...
        try {
            ...
            if (!String(error?.message || '').includes('Conflict: profile was modified by another session')) {
                throw error;
            }
            await this.init();
            result = await runRequest();
    ...
    async saveCurrentProfile() {
        ...
        const runRequest = () => apiFetch('/save-profile', {
            method: 'POST',
