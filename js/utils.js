...
let _rngFn = Math.random; // default: unseeded
...
export function randFloat(min, max) {
    return _rngFn() * (max - min) + min;
}

export function randInt(min, max) {
    return Math.floor(_rngFn() * (max - min + 1)) + min;
}

export function randChoice(arr) {
    return arr[Math.floor(_rngFn() * arr.length)];
}
